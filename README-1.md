...
- Scikit-learn (Logistic Regression, Decision Tree, Linear Regression, Random Forest, K-Means, PCA, preprocessing, metrics)
- Plotly (all interactive charts)
- Matplotlib (used only for the depth-limited decision tree visualization on the Model Evaluation page)
...
- **Compile checks**: all Python files compile cleanly (`python -m py_compile`)
- **Smoke tests** (`tests_stage2_smoke.py`): stubs Streamlit + Plotly and executes the real `app.py` logic for every page (10/10 pages), catching real runtime errors even though visual rendering can't be verified in a headless sandbox
- **Classification verification**: feature exclusion (`salary`/`gender`/`sl_no`/`status`), binary target encoding, valid prediction outputs, and metrics re-derived from the live model
- **Regression verification**: 148-record salary-only subset, feature exclusion, valid positive predictions, MAE/RMSE/R² re-derived live (including the negative R²)
- **Segmentation verification**: feature exclusion, no-NaN transformed matrix, K=2/3/4 label validity, profile counts summing to 215, Elbow/Silhouette computed for K=2–6, PCA projection row count
- **AI Copilot verification**: evidence registry schema validation, 9+ supported question categories, a dedicated hallucination-resistance test ("Which company offered the highest salary?") run through the actual `app.py` UI code path with no API key configured
- **What-If Simulator verification**: 22-item checklist including both-model comparison, single-variable and categorical experiments, change detection (including the zero-change edge case), and min/max input edge cases
